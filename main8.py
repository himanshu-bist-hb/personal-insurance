...
/* ── Sidebar nav: all LOB buttons hidden, rendered as HTML divs ──
   We render each nav item as pure HTML (active state set in Python),
   and place zero-height invisible st.buttons on top for click capture.
   This is 100% reliable — no CSS :has(), no JS, no race conditions.   ── */

/* Completely hide ALL sidebar buttons — visuals are pure HTML */
...
/* Wrapper div needs relative positioning so absolute button aligns */
[data-testid="stSidebar"] [data-testid="stButton"] {
  position: relative !important;
  margin: 0 !important;
  padding: 0 !important;
}
/* Collapse button — restore visibility; it sits AFTER the nav items in DOM order.
   We identify it by its unique key class .st-key-collapse_sidebar_btn */
...
with st.sidebar:
    ...
    ">
      <img
        src="https://1000logos.net/wp-content/uploads/2023/01/Nationwide-logo.png"
      ...
      />
      <div id="sb-fallback" style="
        display:none; font-family:'Libre Baskerville',serif;
        font-size:16px; color:#0D3F7A; font-weight:700; line-height:1.2;
      ">Nationwide<br><span style="color:#C8A951;font-size:14px;">Insurance</span></div>
      <div style="
        margin-top: 10px;
        font-size: 8.5px; font-weight: 700; letter-spacing: 2.5px;
        text-transform: uppercase; color: #98A4B8;
      ">BA Analytics Platform</div>
    </div>
    """, unsafe_allow_html=True)

    # ── Section label ──
    st.markdown("""
    <div style="padding: 18px 20px 6px; font-size: 8.5px; font-weight: 700;
                letter-spacing: 2.5px; text-transform: uppercase; color: #A0AABF;">
      Line of Business
    </div>
    """, unsafe_allow_html=True)

    # ── Nav items: Python renders correct active state, invisible button captures click ──
    active_lob_sb = st.session_state.lob

    for lob_name, icon in LOB_NAV:
        is_active = (lob_name == active_lob_sb)

        if is_active:
            # Active: deep blue bg, white text, left accent bar
            item_html = f"""
            <div style="
              display: flex; align-items: center; gap: 11px;
              padding: 12px 20px; margin: 1px 8px; border-radius: 8px;
              background: linear-gradient(135deg, #1A5DAB 0%, #0D3F7A 100%);
              box-shadow: 0 2px 8px rgba(13,63,122,0.22);
              cursor: pointer;
            ">
              <span style="font-size:16px; line-height:1;">{icon}</span>
              <span style="font-family:'Inter',sans-serif; font-size:13px;
                           font-weight:700; color:#FFFFFF; letter-spacing:0.1px;">
                {lob_name}
              </span>
              <span style="margin-left:auto; width:6px; height:6px; border-radius:50%;
                           background:#EDD97A; flex-shrink:0;"></span>
            </div>"""
        else:
            ...
            " onmouseover="this.style.background='#EEF3FB'"
              ...
              <span style="font-size:16px; line-height:1; opacity:0.65;">{icon}</span>
              <span style="font-family:'Inter',sans-serif; font-size:13px;
                           font-weight:500; color:#5A6A85; letter-spacing:0.1px;">
                {lob_name}
              </span>
            </div>"""

        # Render the visual HTML row
        st.markdown(item_html, unsafe_allow_html=True)

        # Invisible full-size button stacked on top for click detection
        if st.button(f"{lob_name}", key=f"nav__{lob_name}", use_container_width=True):
    ...
    <div style="margin: 20px 12px 0;
                border-top: 1px solid #E8ECF2;"></div>
    """, unsafe_allow_html=True)
    ...
    # ── Footer links ──
    st.markdown("""
    <div style="padding: 12px 20px 20px; font-size: 10px; color: #A0AABF; line-height: 2.4;">
      &#10067;&nbsp; Support<br>
      &#128196;&nbsp; Documentation
    </div>
    """, unsafe_allow_html=True)
