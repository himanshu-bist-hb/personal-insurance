...
/* Hide Streamlit's built-in collapse arrow — sidebar collapses via its own button */
[data-testid="stSidebarCollapseButton"],
[data-testid="collapsedControl"] { display: none !important; }



/* ── Sidebar nav buttons — ALL LOB nav buttons share this base style ── */
...
/* Wrapper: no extra margin between items */
[data-testid="stSidebar"] [data-testid="stButton"] {
  margin: 1px 8px !important;
}

/* ── ACTIVE nav item — targeted by unique key class ── */
[data-testid="stSidebar"] .st-key-nav__Business_Auto button,
[data-testid="stSidebar"] .st-key-nav__General_Liability button,
[data-testid="stSidebar"] .st-key-nav__Farm_Auto button,
[data-testid="stSidebar"] .st-key-nav__Property button {
  /* overridden per-LOB below at render time via a scoped <style> block */
}

/* ── Collapse button ── */
[data-testid="stSidebar"] .st-key-collapse_sidebar_btn button {
...
[data-testid="stSidebar"] .st-key-collapse_sidebar_btn button:hover {
...
with st.sidebar:
    ...
    # ── Logo ──
    st.markdown("""
    <div style="padding:24px 20px 18px; border-bottom:1px solid #E8ECF2;
                background:linear-gradient(160deg,#FFFFFF 60%,#EEF3FB 100%);">
      <img src="https://1000logos.net/wp-content/uploads/2023/01/Nationwide-logo.png"
           alt="Nationwide Insurance"
           style="width:150px; display:block; object-fit:contain;"
           onerror="this.style.display='none';
                    document.getElementById('sb-fallback').style.display='block';" />
      <div id="sb-fallback" style="display:none; font-family:'Libre Baskerville',serif;
           font-size:16px; color:#0D3F7A; font-weight:700; line-height:1.2;">
        Nationwide<br><span style="color:#C8A951;font-size:14px;">Insurance</span>
      </div>
      <div style="margin-top:10px; font-size:8.5px; font-weight:700; letter-spacing:2.5px;
                  text-transform:uppercase; color:#98A4B8;">BA Analytics Platform</div>
    </div>
    <div style="padding:18px 20px 4px; font-size:8.5px; font-weight:700;
                letter-spacing:2.5px; text-transform:uppercase; color:#A0AABF;">
      Line of Business
    </div>
    """, unsafe_allow_html=True)

    # ── NAV: one real st.button per LOB, styled active/inactive via scoped CSS ──
    # The key pattern is  nav__Business_Auto, nav__General_Liability, etc.
    # Streamlit writes class  st-key-nav__Business_Auto  on the parent div.
    # We inject a <style> block ONCE (before the buttons) that targets ONLY
    # the currently-active key.  Every button is a real, clickable native button.
    # This is zero-overlay, zero-JS, zero-latency.

    active_lob_sb  = st.session_state.lob
    active_css_key = "nav__" + active_lob_sb.replace(" ", "_")

    st.markdown(f"""
    <style>
    /* Active LOB button — Nationwide blue gradient */
    [data-testid="stSidebar"] .st-key-{active_css_key} button {{
      background: linear-gradient(135deg, #1A5DAB 0%, #0D3F7A 100%) !important;
      color: #FFFFFF !important;
      font-weight: 700 !important;
      box-shadow: 0 2px 10px rgba(13,63,122,0.25) !important;
      border-radius: 8px !important;
    }}
    [data-testid="stSidebar"] .st-key-{active_css_key} button:hover {{
      background: linear-gradient(135deg, #1A5DAB 0%, #0D3F7A 100%) !important;
      color: #FFFFFF !important;
    }}
    </style>
    ...
    for lob_name, icon in LOB_NAV:
        label = f"{icon}  {lob_name}"
        if st.button(label, key=f"nav__{lob_name.replace(' ', '_')}", use_container_width=True):
            st.session_state.lob        = lob_name
            st.session_state.run_status = "idle"
            st.rerun()

    # ── Divider + collapse ──
    st.markdown("<div style='margin:16px 12px 12px; border-top:1px solid #E8ECF2;'></div>",
                unsafe_allow_html=True)

    if st.button("◀  Collapse", key="collapse_sidebar_btn", use_container_width=True):
        st.markdown("""
        <script>
        (function() {
          var candidates = [
          ...
          for (var i = 0; i < candidates.length; i++) {
            var b = window.parent.document.querySelector(candidates[i]);
            if (b) { b.click(); break; }
          }
        })();
        </script>
    ...
    st.markdown("""
    <div style="padding:10px 20px 20px; font-size:10px; color:#A0AABF; line-height:2.6;">
      &#10067;&nbsp; Support<br>
      &#128196;&nbsp; Documentation
    </div>
    """, unsafe_allow_html=True)
