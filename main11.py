...
/* Remove all gap / padding from the radio container */
[data-testid="stSidebar"] [data-testid="stRadio"] > div { gap:0 !important; padding:0 !important; }
[data-testid="stSidebar"] [data-testid="stRadio"] [data-baseweb="radio"] { margin:0 !important; padding:0 !important; }
...
/* ── SELECTED item — every selector variant, covering all Streamlit versions ── */
...
[data-testid="stSidebar"] [data-testid="stRadio"] label[aria-checked="true"] {
  ...
  box-shadow: 0 3px 12px rgba(13,63,122,0.25) !important;
}
...
/* 2. checked input sibling — works when aria-checked is NOT on the label */
...
with st.sidebar:
    ...
    # Logo
    st.markdown("""
    <div style="padding:24px 20px 18px; border-bottom:1px solid #DDE5F0;
                background:linear-gradient(160deg,#fff 55%,#EEF3FB 100%);">
      <img src="https://1000logos.net/wp-content/uploads/2023/01/Nationwide-logo.png"
           alt="Nationwide Insurance"
           style="width:148px; display:block; object-fit:contain;"
           onerror="this.style.display='none';
                    document.getElementById('sbfall').style.display='block';" />
      <div id="sbfall" style="display:none; font-family:'Libre Baskerville',serif;
           font-size:16px; color:#0D3F7A; font-weight:700; line-height:1.2;">
        Nationwide<br><span style="color:#C8A951;font-size:13px;">Insurance</span>
      </div>
      <div style="margin-top:9px; font-size:8px; font-weight:700; letter-spacing:2.8px;
                  text-transform:uppercase; color:#9BAABF;">BA Analytics Platform</div>
    </div>
    ...
    # JS fallback — directly styles the active label in case CSS selectors miss.
    # Runs immediately after render and again after 300 ms to catch re-renders.
    ...
    (function highlight() {{
      ...
      labels.forEach(function(lbl) {{
        ...
        if (txt.indexOf(active) !== -1) {{
          lbl.style.setProperty('background',
            'linear-gradient(135deg,#1A5DAB 0%,#0D3F7A 100%)', 'important');
          lbl.style.setProperty('color', '#FFFFFF', 'important');
          lbl.style.setProperty('font-weight', '700', 'important');
          lbl.style.setProperty('box-shadow',
            '0 3px 12px rgba(13,63,122,0.25)', 'important');
        }} else {{
    ...
    # Divider + collapse
    st.markdown("<div style='margin:16px 8px 12px; border-top:1px solid #DDE5F0;'></div>",
                unsafe_allow_html=True)

    if st.button("◀  Collapse sidebar", key="collapse_btn", use_container_width=True):
        st.markdown("""<script>
        (function(){
          var s=['button[aria-label="Close sidebar"]',
                 '[data-testid="stSidebarCollapseButton"] button'];
          for(var i=0;i<s.length;i++){
            var b=window.parent.document.querySelector(s[i]);
            if(b){b.click();break;}
          }
        })();
    ...
    st.markdown("""
    <div style="padding:10px 20px 24px; font-size:10px; color:#9BAABF; line-height:2.6;">
      &#10067;&nbsp; Support<br>&#128196;&nbsp; Documentation
    </div>
    """, unsafe_allow_html=True)
