...
/* ── Sidebar nav buttons — flat colored highlight, no box ── */
/* Target all secondary buttons inside the sidebar */
...
/* Active nav item gets blue background fill + right accent */
[data-testid="stSidebar"] [data-testid="baseButton-secondary"].nav-active {{
...
with st.sidebar:
    ...
    for lob_name, cfg in LOB_CONFIG.items():
        ...
        icon = cfg['icon']
        # Inline style for active vs inactive — no box, just flat fill
        active_style = f"""
            background: {PRIMARY_FIXED} !important;
            color: {PRIMARY} !important;
            font-weight: 700 !important;
        ...
        # We render a custom HTML label but use st.button for click handling
        # Active state injected via a JS trick targeting the button by key
        btn_label = f"{icon}  {lob_name}"
        if st.button(btn_label, key=f"nav_{lob_name}", use_container_width=True):
    ...
    # JavaScript to style the active button without a box
    active_key = f"nav_{st.session_state.lob}".replace(" ", "_")
    st.markdown(f"""
    <script>
    (function() {{
      function styleNavButtons() {{
        var buttons = window.parent.document.querySelectorAll('[data-testid="stSidebar"] [data-testid="baseButton-secondary"]');
        buttons.forEach(function(btn) {{
          ...
          btn.style.color = '#424751';
          btn.style.borderRight = '3px solid transparent';
          btn.style.transition = 'background .12s ease';
          btn.onmouseenter = function() {{
            if (!btn.dataset.active) btn.style.background = '#e0e3e6';
          }};
        ...
        var lob = "{st.session_state.lob}";
        buttons.forEach(function(btn) {{
          if (btn.innerText.trim().includes(lob.split(' ')[0]) || btn.innerText.includes(lob)) {{
            btn.style.background = '#d7e2ff';
            btn.style.color = '#003e83';
            btn.style.fontWeight = '700';
            btn.style.borderRight = '3px solid #003e83';
            btn.dataset.active = 'true';
          }}
        }});
      }}
      if (document.readyState === 'complete') styleNavButtons();
      else window.addEventListener('load', styleNavButtons);
      setTimeout(styleNavButtons, 300);
      setTimeout(styleNavButtons, 800);
    }})();
    </script>
    """, unsafe_allow_html=True)
