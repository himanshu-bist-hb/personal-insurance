...
/* Also hide the arrow chevron that Streamlit injects */
section[data-testid="stSidebar"] > div > button {{
    display: none !important;
}}

/* ── Upload zone cards — compact horizontal row style ── */
.upload-card {{
    background: {SURF_LOW};
    border-radius: 8px;
...
/* ── Sidebar nav buttons — hide Streamlit's default button chrome ── */
[data-testid="stSidebar"] [data-testid="baseButton-secondary"] {{
    display: none !important;
}}

/* ── Smooth Streamlit slider — Nationwide blue gradient fill ── */
[data-testid="stSlider"] input[type="range"] {{
    -webkit-appearance: none !important;
    appearance: none !important;
    height: 5px !important;
    border-radius: 3px !important;
    background: linear-gradient(to right, {PRIMARY} 0%, {PRIMARY_CONT} 0%, {SURF_HIGH} 0%) !important;
...
[data-testid="stSlider"] input[type="range"]::-webkit-slider-thumb {{
    ...
    box-shadow: 0 1px 6px rgba(0,62,131,.35) !important;
    cursor: grab !important;
    transition: transform .1s ease, box-shadow .1s ease !important;
}}
[data-testid="stSlider"] input[type="range"]::-webkit-slider-thumb:active {{
    cursor: grabbing !important;
    transform: scale(1.2) !important;
    box-shadow: 0 2px 12px rgba(0,62,131,.5) !important;
}}
...
/* Hide Streamlit's default slider value bubble */
...
/* Make the uploader text smaller */
...
/* Hide the sidebar collapse toggle in all its forms */
...
with st.sidebar:
    ...
    <div style="padding: 0 20px 20px 20px;">
      <div style="font-family:'Manrope',sans-serif; font-size:20px; font-weight:800;
                  color:#003e83; letter-spacing:-.5px; line-height:1;">
        Nationwide
      </div>
      <div style="font-size:9px; font-weight:700; letter-spacing:.14em;
                  text-transform:uppercase; color:#9aa0ae; margin-top:3px;">
        BA Analytics Platform
      </div>
    </div>
    <div style="font-size:9px; font-weight:700; letter-spacing:.12em;
                text-transform:uppercase; color:#9aa0ae;
                padding: 0 20px 10px 20px; border-bottom:1px solid #c3c6d344;">
      Insurance Categories
    </div>
    """, unsafe_allow_html=True)

    # ── LOB radio nav — styled to look like a sidebar menu ──
    # st.radio is the most reliable Streamlit selection widget.
    # We style away the radio circles and make each option look like a nav row.
    st.markdown("""
    <style>
    ...
    /* Selected radio option — Nationwide blue */
    ...
    [data-testid="stSidebar"] [data-testid="stRadio"] input:checked + div {
        ...
        font-weight: 700 !important;
        border-right: 4px solid #abc7ff !important;
    }
    /* Target the checked label via sibling CSS */
    ...
    /* Remove stRadio container padding/gap */
    ...
    lob_options = ["🚗  Business Auto", "⚖️  General Liability", "🚜  Farm Auto", "🏠  Property"]
    lob_names   = ["Business Auto", "General Liability", "Farm Auto", "Property"]

    # Map current LOB to the display option
    ...
    # Extract the LOB name from the selected option
    selected_lob = lob_names[lob_options.index(selected)]
    if selected_lob != st.session_state.lob:
        st.session_state.lob         = selected_lob
    ...
    <div style='padding: 4px 20px 8px 20px;'>
      <div style='font-size:11px; color:#9aa0ae; display:flex; gap:8px;
...
with tab_dashboard:
    ...
    if not cfg["mandatory_files"] and not cfg["optional_files"]:
        ...
        with col_files:
            ...
            # Grid of file uploaders — compact 2-column list layout
            n_cols = 2
            ...
            for row_items in mand_chunks:
                ...
                for col, (key, label, fmt) in zip(cols, row_items):
                    with col:
                        ...
                        border  = f"border-color:{PRIMARY}; border-style:solid;" if is_up else ""
                        fname   = f"<span class='file-pill'>{already.name}</span>" if is_up else f"<span class='upload-card-sub'>{fmt}</span>"
                        st.markdown(f"""
        ...
        with col_right:
            ...
            cur_sched = st.session_state.get("sched_rating", 0)

            # Styled live-display box above the slider
            st.markdown(f"""
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
              <div id="sched-display" style="
                background:#ffffff; border:1.5px solid #c3c6d3; border-radius:6px;
                padding:6px 12px; font-size:14px; font-weight:700; color:#003e83;
                min-width:64px; text-align:center; font-family:'Manrope',sans-serif;
              ">{cur_sched}</div>
              <span style="font-size:12px; color:#8a8fa0; font-weight:600;">/ 100</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            new_sched = st.slider(
                ...
                value=cur_sched,
                step=1,
                key="sched_slider_main",
                label_visibility="collapsed",
            )
            st.session_state.sched_rating = new_sched

            # JS: live-update display box as user drags without any rerun
            st.markdown(f"""
            <script>
            (function() {{
              function syncDisplay() {{
                var sliders = window.parent.document.querySelectorAll('[data-testid="stSlider"] input[type="range"]');
                var display = window.parent.document.getElementById('sched-display');
                if (!sliders.length || !display) return;
                var sl = sliders[sliders.length - 1];
                display.textContent = sl.value;
                var pct = (sl.value / sl.max) * 100;
                sl.style.background = 'linear-gradient(to right, #003e83 0%, #1d56a4 ' + pct + '%, #e0e3e6 ' + pct + '%, #e0e3e6 100%)';
                sl.addEventListener('input', function() {{
                  display.textContent = this.value;
                  var p = (this.value / this.max) * 100;
                  this.style.background = 'linear-gradient(to right, #003e83 0%, #1d56a4 ' + p + '%, #e0e3e6 ' + p + '%, #e0e3e6 100%)';
                }});
              }}
              setTimeout(syncDisplay, 200);
              setTimeout(syncDisplay, 700);
              setTimeout(syncDisplay, 1400);
            }})();
            </script>
            """, unsafe_allow_html=True)

            st.markdown("""
            <p style='font-size:9px; color:#9aa0ae; margin-top:-4px; margin-bottom:12px;'>
