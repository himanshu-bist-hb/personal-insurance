...
with st.sidebar:
    ...
    # ── Nav items: pure HTML for styling, st.button (invisible) for clicks ──
    # Each LOB gets a styled div rendered via HTML + a zero-height st.button stacked on top
    ...
    for lob_name in LOB_CONFIG.keys():
        is_active = active_lob == lob_name
        icon = NAV_ICONS[lob_name]
        ...
        if is_active:
            ...
            <div style="
              ...
              padding:11px 20px;
              background:#003e83;
              border-right:4px solid #abc7ff;
              cursor:pointer;
              margin-bottom:1px;
            ">
              <span style="font-size:16px;">{icon}</span>
              <span style="font-family:'Inter',sans-serif; font-size:13px;
                           font-weight:700; color:#ffffff; letter-spacing:.01em;">
                {lob_name}
              </span>
            </div>
            """
            ...
            " onmouseover="this.style.background='#e0e3e6'"
               onmouseout="this.style.background='transparent'">
              <span style="font-size:16px;">{icon}</span>
              <span style="font-family:'Inter',sans-serif; font-size:13px;
                           font-weight:500; color:#424751;">
                {lob_name}
              </span>
            </div>
            """
        ...
        # Invisible button overlaid — zero visual footprint, just captures the click
        st.markdown("""
        <style>
        div[data-testid="stButton"] > button {
          position: relative; margin-top: -46px; opacity: 0;
        ...
        if st.button("_", key=f"nav_btn_{lob_name}", use_container_width=True):
